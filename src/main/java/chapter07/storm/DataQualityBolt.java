...
public class DataQualityBolt implements IBasicBolt{
	...
	public void execute(Tuple input, BasicOutputCollector collector) {
		...
		try {
			...
			currentcnt=tableCnt.incrementColumnValue(Bytes.toBytes(newtime), Bytes.toBytes("cf1"), 
					Bytes.toBytes("cnt"), 1L);
			System.out.println("s2-->"+currentcnt+"  "+Long.toString(currentcnt));
		...
		if(cellphonenumber.equals("")){
			try {
				emportynumber=tableCnt.incrementColumnValue(Bytes.toBytes(newtime), Bytes.toBytes("cf1"), Bytes.toBytes("emptynumber"), 1L);
				System.out.println("s3-->"+emportynumber+"  "+Long.toString(emportynumber));
